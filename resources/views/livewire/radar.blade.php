...
<div>
    <section class="our-product pt-2 pb-2">
        ...
        <form wire:submit.prevent="addToCart">
            ...
            <input type="hidden" wire:model="category" name="category" id="category"
                   value="{{ $product->category->title }}">
            <input type="hidden" wire:model="price" name="price" id="price"
                   value="{{ $country_code=="CA"? $product->price_canada*$exchange_rate:$product->price*$exchange_rate }}">
            <input type="hidden" wire:model="cover_image" name="cover_image" id="cover_image"
    ...
    handleChange(specId) {
        this.error = this.dynamic_specs[specId] ? '' : 'This field is required.';
        this.isSpecSelected = Object.values(this.dynamic_specs).every(value => value !== 0); // Check if all specifications are selected
    },
                                ...
                                <div class="responsive-two">
                                    <div>
                                        <div class="p-2 v_zoom-product-container" id="slider_static">
                                            <div
                                                class="img-leften d-flex justify-content-center align-items-center v_zoom-image-container">
                                                <img
                                                    src="{{ asset('storage/' . ($product->images[0]->image ?? $product->cover_image)) }}"
                                                    class="thumb img-fluid" style="max-height: 600px;"
                                                    id="big-img-radar-product" alt="{{ $product->title }}" wire:ignore>
                    ...
                    <div class="col-lg-6">
                        <div class="multiple-optionn pb-0 pt-lg-0 pt-5 pb-0">
                            ...
                            @if($product->is_price_hide != 1)
                                <p class="fs-2" id="total_price2"><span
                                        x-ref="total_price">{{$currency_icon}}{{ $price }}</span></p>
                            @else
                            ...
                            <div class="row">
                                ...
                                @foreach ($product->specilizations->reverse() as $specilization)
                                    <div class="col-md-8 bg-transparent">
                                        ...
                                        <h6 class="text-dark"
                                            style="font-weight: bold;">{{ $specilization->specilization->title }}<span class="text-danger"> *</span></h6>

                                        @if($specilization->specilization->code == "CR")
                                            <div class="d-flex justify-content-center">
                                                <select
                                                    x-on:change="changeCalculatedAmount({{ $specilization->id }}, $event.target)"
                                                    ...
                                                    wire:ignore
                                                    required
                                                    @change="handleChange({{ $specilization->id }})"
                                                    @blur="handleBlur({{ $specilization->id }})"
                                                    :aria-invalid="error ? 'true' : 'false'">
                                                    <option selected>--Choose an Option--</option>
                                                    @foreach($specilization->options as $option)
                                                        <option value="{{ $option->id }}"
                                                                data-code="{{ $option->specializationoptions->code }}">
                                                            {{ $option->specializationoptions->option }}
                                                            @if($country_code=="CA")
                                                                (+$<span
                                                                    class="price">{{ $option->specialization_price_ca * $exchange_rate }}</span>
                                                                )
                                                            @else
                                                ...
                                                <!-- Error message -->
                                                <span x-show="error" class="text-red-500 text-sm mt-2"
                                                      x-text="error"></span>

                                                <!-- Image for color or spec -->
                                                <span>
                        <img src="{{ asset('/assets/images/radar/color/AM.png') }}" style="height:40px;"
                             id="imgicon_color_st-{{ $specilization->id }}" alt="color" wire:ignore/>
                    </span>
                                                ...
                                                <script>
                                                    document.addEventListener('DOMContentLoaded', function () {
                                                        ...
                                                        selectElement.addEventListener('change', function () {
                                                            ...
                                                            const baseUrl = '{{ env('APP_URL', 'http://127.0.0.1:8000/') }}';
                                                            if (code) {
                                                                const newImageSrc = `${baseUrl}/assets/images/radar/color/${code}.png`;
                                    ...
                                    document.addEventListener('DOMContentLoaded', function () {
                                        ...
                                        function updateSKU() {
                                            ...
                                            const skuParts = Array.from(selects).map(select => {
                                                const specializationCode = select.getAttribute('data-code');
                                                const selectedOption = select.options[select.selectedIndex];
                                                const optionCode = selectedOption ? selectedOption.getAttribute('data-code') : '';
                                                return specializationCode + (optionCode ? '/' + optionCode : '');
                                            }).filter(Boolean);
                        ...
                        <div class="col-lg-8 col-md-8">
                            <div class="d-md-flex justify-content-start mt-lg-0 mt-4 buy-right align-items-center">
                                <div x-data="{ quantity: @entangle('quantity') }"
                                     class="d-flex align-items-center border p-2"
                                     style="background-color: white; box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.1);">
                                ...
                                <button type="submit"
                                        ...
                                        class="btn rounded-0 text-nowrap align-self-center px-4 m-2"
                                        data-bs-toggle="modal"
                                        data-bs-target="#exampleModalCenter"
                                        id="add-to-cart-button"
                                        :title="!isSpecSelected ? 'Please select specifications' : ''">
                                    <img style="height: 58px;" class="img_size" src="{{ asset('assets/images/add_to_cart.webp') }}">
                                </button>

                            </div>
                        </div>
                        <p class="mt-4">{{$product->shipping_text??"Comes with multiple power options such as Standalone Solar powered operations. <br> Shipping: 7-10 Working Days"}}
                            .</p>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header d-flex justify-content-between align-items-center">
                        ...
                        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Continue
                            Shopping
                        </button>
                    </div>
                    <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                        ...
                        @foreach($cartItems as $item)
                            ...
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <div
                                    class="ms-auto text-black">{{$currency_icon}}{{ isset($item->price) ? number_format($item->price, 2) : '' }}</div>
                            </div>
                            <hr>
                        @endforeach
                        <script>
                    ...
                    <div class="modal-footer">
                        <button wire:click="navigateToShopping" class="btn btn-primary w-100">View Cart / Checkout
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class='row'>
                <div class='col-lg-9'>
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link text-dark" id="profile-tab" data-bs-toggle="tab"
                                    ...
                                    aria-controls="specification-tab-pane" aria-selected="false">
                                Specifications
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade mt-3" id="specification-tab-pane" role="tabpanel"
                             aria-labelledby="specification-tab-pane" tabindex="0">
                            {!! $product->specification!!}
                        </div>
                        <div class="tab-pane fade mt-3" id="shipping-tab-pane" role="tabpanel"
                             aria-labelledby="shipping-tab-pane" tabindex="0">
                            {!! $product->feature!!}
                        </div>
                    </div>
                </div>
                <div class="col-lg-3 pt-4">
                    <div class="row">
                        ...
                        <div class="container mt-5">
                            ...
                            <p class="note">
                                Note: All above features <span>worth $1450</span> are included in this product without
                                any extra/hidden cost (limited time offer).
                            </p>
                        </div>
                    </div>
                            ...
                            @foreach($linked_products as $ap)
                                <div class="col-md-12 mb-3">
                                    <form wire:submit.prevent="addAccessory">
                                        ...
                                        <div class="card">
                                            <div
                                                class="d-flex justify-content-center align-items-center w-100 p-4 pb-2 border-1">
                                                <img x-init="@this.set('cover_image_a', {{$ap->cover_image}})"
                                                     src="{{ asset('storage/' . $ap->cover_image )  }}"
                                                     class="card-img-top"
                                                     alt="{{$ap->title}}"
                                                     style="max-width: 100%; ">
                                            </div>
                                            ...
                                            <div class="card-body">
                                                ...
                                                <p class="card-text"
                                                   x-init="@this.set('price_a', {{$country_code == 'CA' ? ($ap->price_canada * $exchange_rate) : ($ap->price * $exchange_rate)}})">
                                                    {{ $currency_icon ?? '$' }}
                                                    {{ $country_code == 'CA' ? ($ap->price_canada * $exchange_rate) : ($ap->price * $exchange_rate) }}
                                                </p>

                                                <p class="text-muted p-2"
                                                   x-init="@this.set('category_a', {{$ap->category->title}})">{{$ap->category->title}}</p>

                                                <input type="hidden" x-init="@this.set('Pid_a', {{$ap->id}})"
                                                       name="product_id" value="{{ $ap->id }}">
                                                <input type="hidden" name="price"
                                                       value="{{ $country_code == "CA" ? $ap->price_canada * $exchange_rate : $ap->price * $exchange_rate }}">
                                                <input type="hidden" wire:model="title_a" name="title"
                                                       value="{{ $ap->product_heading_text ?? $ap->title }}">
                                                <input type="hidden" wire:model="title_a" name="title"
                                                       value="{{ $ap->product_heading_text ?? $ap->title }}">
                                                ...
                                                <div
                                                    class="quantity-control d-flex align-items-center justify-content-center gap-2">
                                                    <button type="button"
                                                            class="btn btn-sm btn-outline-secondary decrement-btn p-2 pb-1 pt-1"
                                                            style="width: 40px;max-width: 100%;">-
                                                    </button>
                                                    <input type="number" wire:model="quantity_a" name="quantity"
                                                           value="1" class="form-control text-center quantity-input p-1"
                                                           style="width: 60px; height: 30px;" min="1" readonly>
                                                    <button type="button"
                                                            class="btn btn-sm btn-outline-secondary increment-btn p-2 pb-1 pt-1"
                                                            style="width: 40px;max-width: 100%;">+
                                                    </button>
                                                </div>
                                                ...
                                                <button data-bs-toggle="modal" data-bs-target="#exampleModalCenter"
                                                        type="submit" type="submit" class="btn btn-primary mt-3">Add to
                                                    Cart
                                                </button>
                                            </div>
                                        </div>
